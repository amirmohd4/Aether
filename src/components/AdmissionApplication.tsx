...
import { School, Search, CircleAlert as AlertCircle } from 'lucide-react';
...
export function AdmissionApplication() {
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const result = await directFetch<any>('/admission/apply', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleStatusCheck = async () => {
    ...
    setError('');
    try {
      const result = await directFetch<any>(`/admission/status/${appId}`);
      setApp(result);
    } catch (err: any) {
      setError(err.message);
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex gap-3">
          <input
            ...
            placeholder="Enter Application ID (e.g., ADM-XXXXXXXX)"
      ...
      {app && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <p><span className="font-medium">Enrolled At:</span> {app.enrolled_at ? new Date(app.enrolled_at).toLocaleDateString() : 'Not yet'}</p>
          </div>
        </div>
      )}
    </div>
