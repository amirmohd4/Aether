...
import { Building, Search, CircleAlert as AlertCircle } from 'lucide-react';
...
interface Housing { application_id: string; applicant_name: string; status: string; allotted_at: string | null; }

export function AffordableHousingApplication() {
  const [app, setApp] = useState<Housing | null>(null);
  ...
  const [tenantType, setTenantType] = useState('ownership');
  const [monthlyRent, setMonthlyRent] = useState('');

  const handleApply = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true); setError('');
    try {
      const result = await directFetch<any>('/housing/apply', {
        method: 'POST', body: JSON.stringify({ citizen_id: citizenId, applicant_name: applicantName, address, district, state, annual_income: parseFloat(annualIncome) || 0, family_size: parseFloat(familySize) || 0, preferred_tenant_type: tenantType, monthly_rent_affordable: parseFloat(monthlyRent) || 0 }),
      });
      setApp(result); setAppId(result.application_id); alert('✅ Affordable housing application submitted!');
  ...
  const handleStatusCheck = async () => {
    ...
    try { const result = await directFetch<Housing>(`/housing/status/${appId}`); setApp(result); } catch (err: any) { setError(err.message); } finally { setLoading(false); }
  };

  const getStatusColor = (status: string) => ({ applied: 'bg-yellow-100 text-yellow-800', shortlisted: 'bg-blue-100 text-blue-800', allotted: 'bg-green-100 text-green-800', rejected: 'bg-red-100 text-red-800' }[status] || 'bg-gray-100 text-gray-800');

  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Building className="w-6 h-6 text-blue-600" />Affordable Housing Application</h2>
        ...
        <form onSubmit={handleApply} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700">Family Size</label><input type="number" value={familySize} onChange={(e) => setFamilySize(e.target.value)} className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg" /></div>
            <div><label className="block text-sm font-medium text-gray-700">Preferred Tenant Type</label><select value={tenantType} onChange={(e) => setTenantType(e.target.value)} className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg"><option value="ownership">Ownership</option><option value="rental">Rental</option></select></div>
            <div><label className="block text-sm font-medium text-gray-700">Monthly Rent Affordable (₹)</label><input type="number" value={monthlyRent} onChange={(e) => setMonthlyRent(e.target.value)} className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg" /></div>
          ...
          <button type="submit" disabled={loading} className="w-full py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">{loading ? 'Submitting...' : 'Apply for Affordable Housing'}</button>
        </form>
      </div>
      <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      {app && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <p><span className="font-medium">Allotted At:</span> {app.allotted_at ? new Date(app.allotted_at).toLocaleDateString() : 'Not allotted'}</p>
          </div>
        </div>
      )}
    </div>
